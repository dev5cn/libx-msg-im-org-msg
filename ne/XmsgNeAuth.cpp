...
void XmsgNeAuth::handle(shared_ptr<XscChannel> channel, SptrXitp trans, shared_ptr<XmsgNeAuthReq> req)
{
	...
	rsp->set_cgt(XmsgImOrgCfg::instance()->cfgPb->cgt());
	trans->end(rsp);
	if (old == nullptr)
		return;
...
shared_ptr<XmsgImOrgCfgXmsgNeN2hAddr> XmsgNeAuth::findXmsgNeCfg(const string& cgt)
{
	for (int i = 0; i < XmsgImOrgCfg::instance()->cfgPb->n2h_size(); ++i)
	{
		auto ne = XmsgImOrgCfg::instance()->cfgPb->n2h(i);
		if (ne.cgt() != cgt)
			continue;
		shared_ptr<XmsgImOrgCfgXmsgNeN2hAddr> pb(new XmsgImOrgCfgXmsgNeN2hAddr());
		pb->CopyFrom(ne);
		return pb;
	}
	return nullptr;
